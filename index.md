...
![](assets/F3gM_v_No1ioRRZvJ9znGjumM61DL_2B98PRj2B0o7s=.png)
...
* 树
  ...
  ![](assets/utRKzDNEDqKsJNVVhJMC34EsV2uP_OuFt-h6cjZ5IvY=.png)


  1. p点正交投影到前面找的三个最近的view。能采样到三个纹素。通过公式插值。深度加权求和再除以透明度加权求和，这样p往v的方向偏移这段距离就得到点q
     $$
     q=p+(\sum{w_iz_i})/(\sum{w_i\alpha_i})v
     $$
  2. 点q也正交投影到那三个view，再采样三个纹素。由公式插值。$p_v$用于片元depth计算。
     $$
     p_v=p+(\sum{w_iz^,_i}/\sum{w_i\alpha^,_i})v
     $$
  ![](assets/l4ZqBGewhIM6xch4u02X5moqzKbkGNXSlpYPtBQn2DI=.png)


  1. 重复前3步，不过视线方向改成了光源方向，找到$p_l$,这里光线经过前面的$p_v$
  ![](assets/KVUC4Dt1nVxt6q3OpeeVBPTeBe2fktH16Tw_fzux9AU=.png)

  $p_v$的透明度$\alpha_v=\sum{w_i\alpha^,_i}$及AO$\sum{w_i\delta^,_i}/\alpha_v$用于光照计算。



* 太阳光。模型中森林受太阳光由以下三个系数决定
  ...
  $$
  $$
  F(v,l)=^{def}1-a_1[1-exp(-a_2cos^-1(v\cdot l)]
  $$
...
$$
J_t(p_v)=\frac{p}{2}(\int_{\Omega^2}p(v,w)dw)\frac{\delta_v(p_v)(1+n\cdot u_z)}{1+d^2(p_v)\Lambda_h(x)/4}L_{sky}
$$
...
* 地面光
  ...
  $A$average horizontal tree area E\[πR 2 ]
  ...
  $$
  \Delta(x)=^{def}=1-a_3[\Gamma(x)-\Lambda_h(x)A]
  $$
  ...
  $$
  $$
  J_g(x)=\delta_g \Delta(x) \int_{\Omega^+}r(v,w,u_z)u_z\cdot wdwL_{sky}
  $$
* 远处树的表达和渲染

  用shader-map表达，terrain map存了俯视角中树和地面的比例。树的像素的重建用4个view-light相关的系数
...
![](assets/Qf6UUem1CpEjaJ-v78JtnIyu4imW4JOOUb4X-78sc6M=.png)
